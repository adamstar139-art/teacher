...
# كود التنسيق الجمالي المتقدم ودعم RTL
st.markdown("""
<style>
    .main { direction: rtl; }
...
# ==========================================
# 4. ترويسة البرنامج الرئيسية والتبويبات
# ==========================================
st.markdown("""
<div style="text-align:center; padding:15px; background: linear-gradient(135deg, #1e3a8a 0%, #0d9488 100%); color:white; border-radius:15px; margin-bottom:20px;">
    <h1 style="margin:0; font-size:28px;">🏫 مدارس الثغر النموذجية الأهلية - القسم المتوسط</h1>
    <h3 style="margin:5px 0 0 0; font-weight:400; font-size:18px;">نظام المتابعة الصفية والتقييم الإشرافي للمعلمين</h3>
</div>
""", unsafe_allow_html=True)
...
with tab1:
    ...
    with col_sub3:
        lesson_title = st.text_input("عنوان الدرس:", key=f"lesson_title_{cnt}")
    ...
    if submit_save:
        ...
        c.execute('''
            INSERT INTO evaluations (
                teacher_name, specialization, assigned_subjects, selected_subject, lesson_title, grade, class_name,
        ...
        ''', (
            selected_teacher, teacher_spec, all_subs_str, selected_subject, lesson_title, selected_grade, selected_class,
        ...
        if btn_start or st.session_state.get('last_searched_id_v7'):
            ...
            if not df_search.empty:
                ...
                with col_info3:
                    ...
                    st.write(f"📖 **عنوان الدرس:** {rec.get('lesson_title', 'غير محدد')}")
                ...
                if show_edit or st.session_state.get(f'editing_v7_{rec_id}'):
                    ...
                    with st.form(f"edit_form_v7_{rec_id}"):
                        new_lesson_title = st.text_input("عنوان الدرس المعدل:", value=rec.get('lesson_title', ''))
                        ...
                        if save_changes:
                            conn = sqlite3.connect(DB_FILE)
                            c = conn.cursor()
                            c.execute("UPDATE evaluations SET lesson_title = ?, notes = ?, total_score = ? WHERE id = ?", (new_lesson_title, new_notes, new_total, rec_id))
                            conn.commit()
                            conn.close()
